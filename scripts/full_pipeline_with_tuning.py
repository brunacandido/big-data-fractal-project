...
def main(args):
    ...
    df_train = df_train.sample(False, 0.01, seed=42) 
    df_test = df_test.sample(False, 0.01, seed=42)
    df_val  = df_val.sample(False, 0.01, seed=42)
    ...
    df_val  = df_val.select(*final_cols)




    # Create feature vector
    feature_cols = [c for c in df_train.columns if c != "Classification"]
    assembler = VectorAssembler(inputCols=feature_cols, outputCol="features", handleInvalid="skip")
    rf = RandomForestClassifier(labelCol="Classification", featuresCol="features")

    # Full pipeline with assembler + RF
    full_pipeline = Pipeline(stages=[assembler, rf])
    ...
    cv = CrossValidator(
        ...
        evaluator=evaluator_acc,  # primary metric for selecting best model
        numFolds=1,
        parallelism=2
    )
